...
import com.duangframework.scan.ScanClassFactory;
import com.duangframework.kit.DataType;
import com.duangframework.vtor.annotation.Bean;
import com.duangframework.vtor.annotation.Entity;
...
public final class VtorFactory {
	...
	private static Map<Class<?>, AbstractValidatorTemplate> VALIDATOR_HANDLE_MAP = new HashMap<Class<?>, AbstractValidatorTemplate>();

	private static void init() {
		if(ToolsKit.isEmpty(VALIDATOR_HANDLE_MAP)) {
			String packagePath = AbstractValidatorTemplate.class.getPackage().getName();
			List<Class<?>> validatorHandleList = ScanClassFactory.getAllClass(packagePath);
			for (Class<?> clazz : validatorHandleList) {
				if (!ToolsKit.supportInstance(clazz)) {
					continue;
				}
